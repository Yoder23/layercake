"""Construct-certify the generic progressive replacement host v8."""
...
from layercake_extensions.progressive_replacement_core import (
    PROGRESSIVE_REPLACEMENT_CORE_ABI_SHA256,
    PROGRESSIVE_REPLACEMENT_CORE_ABI_VERSION,
    ProgressiveReplacementCoreHost,
)
...
def load_protocol(root: Path, path: Path):
    ...
    if (
        protocol.get("format") != "layercake-postrelease-progressive-replacement-core-construct/1"
        or protocol.get("status") != "PREREGISTERED_CONSTRUCT_ONLY"
    ):
        raise RuntimeError("progressive replacement construct governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha(target) != expected:
            raise RuntimeError(f"progressive replacement construct binding changed: {relative}")
    return protocol, sha(path)


def fixture_module(root: Path):
    path = root / "tests/models/test_progressive_replacement_core.py"
    spec = importlib.util.spec_from_file_location("progressive_replacement_fixture", path)
...
def execute(root: Path, protocol_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="lc-progressive-replacement-v8-") as name:
        temp = Path(name)
        package, public, key_id, state_hash, tokenizer = fixture._package(temp / "source")
        archive_sha = sha(package)
        cpu = ProgressiveReplacementCoreHost(temp / "cpu", trust_store={key_id: public})
        ...
        if torch.cuda.is_available():
            host = ProgressiveReplacementCoreHost(temp / "cuda", trust_store={key_id: public}, device="cuda")
        ...
        checks = {
            "abi_identity": PROGRESSIVE_REPLACEMENT_CORE_ABI_VERSION == "lc-direct-neural-core/8"
            and PROGRESSIVE_REPLACEMENT_CORE_ABI_SHA256 == protocol["interface_sha256"],
            "canonical_abi_hash": sha(root / protocol["canonical_abi"])
            == PROGRESSIVE_REPLACEMENT_CORE_ABI_SHA256,
            "preregistered_target_parameter_count": target_parameters == 253_836_288,
            "zero_source_transformer_blocks": protocol["source_transformer_blocks"] == 0,
            ...
            "persistent_rotary_kv_state": len(initial_cache) == 2
            and all(after == before + 1 for before, after in zip(initial_cache, expanded_cache)),
            "persistent_state_realizes_bytes": isinstance(realized, bytes),
            "cpu_generation_strict": generated == b"",
            "lifecycle_identity": archive_sha == sha(package)
            ...
            and cuda["payload_hash"] == active["payload_hash"]
            and cuda["state_dict_hash"] == active["state_dict_hash"]
            and cuda["generated_hex"] == generated.hex(),
        }
    result = {
        "format": "layercake-postrelease-progressive-replacement-core-construct-result/1",
        ...
        "claim_boundary": "Generic progressive replacement host construct only; no external acquisition, English quality, or performance claim.",
    }
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.command == "execute":
        if output.exists():
            raise RuntimeError("progressive replacement construct result immutable")
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    elif json.loads(output.read_text(encoding="utf-8")) != result:
        raise RuntimeError("stored progressive replacement construct differs")
