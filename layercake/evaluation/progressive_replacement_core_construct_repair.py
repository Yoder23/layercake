"""Correct the v8 exact target geometry to the deployable runtime vocabulary."""
...
def execute(root: Path, protocol_path: Path) -> dict[str, Any]:
    ...
    if (
        protocol.get("format") != "layercake-postrelease-progressive-replacement-core-construct-repair/1"
        or protocol.get("status") != "PREREGISTERED_CONSTRUCT_ACCOUNTING_REPAIR"
    ):
        raise RuntimeError("progressive replacement construct repair governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha(target) != expected:
            raise RuntimeError(f"progressive replacement construct repair binding changed: {relative}")
    base = execute_base(root, root / protocol["base_protocol"])
    runtime_vocabulary = int(protocol["corrected_target_geometry"]["runtime_vocabulary"])
