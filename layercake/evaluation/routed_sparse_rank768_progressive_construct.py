"""Construct verifier for routed sparse rank-768 host v15."""
...
def execute(root: Path, protocol_path: Path) -> dict:
    ...
    if (
        protocol.get("format")
        != "layercake-postrelease-routed-sparse-rank768-progressive-construct/1"
    ...
    spec = importlib.util.spec_from_file_location(
        "fixture", root / "tests/models/test_routed_sparse_rank768_progressive_core.py"
    )
    fixture = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(fixture)
    with tempfile.TemporaryDirectory(prefix="lc-v15-") as directory:
        temp = Path(directory)
        package, public, key_id, state_hash, tokenizer = fixture._package(temp / "source")
        archive = sha(package)
        cpu = RoutedSparseRank768ProgressiveCoreHost(
            temp / "cpu", trust_store={key_id: public}
        )
        active = cpu.activate(package)
        ids = tokenizer.encode_source("hello world")[0]
        state = cpu.prefill("hello world")
        first = torch.allclose(
            state.next_logits,
            cpu.module.forward_routed(torch.tensor([ids]), state.route_index)[:, -1],
            atol=1e-5,
            rtol=1e-5,
        )
        before = tuple(value.shape[2] for value in state.layer_keys)
        state.next_logits.zero_(); state.next_logits[0, 4] = 1
        action, state = cpu.decode_step(state)
        second = torch.allclose(
            state.next_logits,
            cpu.module.forward_routed(torch.tensor([ids + [action]]), state.route_index)[:, -1],
            atol=1e-5,
            rtol=1e-5,
        )
        after = tuple(value.shape[2] for value in state.layer_keys)
        poison = fixture._model(tokenizer).eval()
        with torch.no_grad():
            poison.router.weight.zero_(); poison.router.bias.copy_(torch.tensor([3.0, 2.0, 1.0]))
            for layer in poison.layers:
                layer.route_coefficient_projections[1].weight.fill_(float("nan"))
                layer.route_coefficient_projections[2].weight.fill_(float("nan"))
        physical_sparse = bool(torch.isfinite(poison(torch.tensor([ids]))).all())
        verified = cpu.verify(); cpu.remove(); reinstall = cpu.activate(package)
        cuda = None
        if torch.cuda.is_available():
            cuda = RoutedSparseRank768ProgressiveCoreHost(
                temp / "cuda", trust_store={key_id: public}, device="cuda"
            ).activate(package)
        count = RoutedSparseRank768ProgressiveCore.parameter_count_for_config(
        ...
        checks = {
            "abi": ROUTED_SPARSE_RANK768_ABI_VERSION == "lc-direct-neural-core/15"
            and ROUTED_SPARSE_RANK768_ABI_SHA256 == protocol["interface_sha256"],
            "abi_hash": sha(root / protocol["canonical_abi"])
            == ROUTED_SPARSE_RANK768_ABI_SHA256,
            "parameters": count == 536_758_275,
            "routes": cpu.module.route_names == ("generic", "abstention", "conversation"),
            "dual_cache": all(value.shape[1] == 2 * cpu.module.attention_heads for value in state.layer_keys),
            "physical_sparse": physical_sparse,
            "first": bool(first),
            "second": bool(second),
            "cache": all(right == left + 1 for left, right in zip(before, after)),
            "persistent_route": hasattr(state, "route_index") and state.route_index == 0,
    ...
    result = {
        "format": "layercake-postrelease-routed-sparse-rank768-progressive-result/1",
        ...
        "claim_boundary": "Generic routed sparse rank768 host construct only; no ABI acquisition, quality, runtime, or superiority claim.",
    }
