"""Construct verifier for the source-aligned dual-path progressive host v10."""
...
from layercake_extensions.dual_path_progressive_core import (
    DUAL_PATH_PROGRESSIVE_ABI_SHA256,
    DUAL_PATH_PROGRESSIVE_ABI_VERSION,
    DualPathProgressiveCoreHost,
)
...
def execute(root: Path, protocol_path: Path) -> dict[str, Any]:
    ...
    if (
        protocol.get("format") != "layercake-postrelease-dual-path-progressive-construct/1"
        or protocol.get("status") != "PREREGISTERED_CONSTRUCT_ONLY"
    ):
        raise RuntimeError("dual-path progressive construct governance changed")
    for relative, expected in protocol["bindings"].items():
        target = root / relative
        if not target.is_file() or sha(target) != expected:
            raise RuntimeError(f"dual-path progressive binding changed: {relative}")

    path = root / "tests/models/test_dual_path_progressive_core.py"
    spec = importlib.util.spec_from_file_location("dual_path_fixture", path)
    fixture = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(fixture)
    with tempfile.TemporaryDirectory(prefix="lc-dual-path-v10-") as name:
        temp = Path(name)
        package, public, key_id, state_hash, tokenizer = fixture._package(temp / "source")
        archive_hash = sha(package)
        cpu = DualPathProgressiveCoreHost(temp / "cpu", trust_store={key_id: public})
        ...
        if torch.cuda.is_available():
            host = DualPathProgressiveCoreHost(temp / "cuda", trust_store={key_id: public}, device="cuda")
            cuda = host.activate(package)

        target_parameters = DualPathProgressiveCore.parameter_count_for_config(
        ...
        checks = {
            "abi_identity": DUAL_PATH_PROGRESSIVE_ABI_VERSION == "lc-direct-neural-core/10"
            and DUAL_PATH_PROGRESSIVE_ABI_SHA256 == protocol["interface_sha256"],
            "canonical_abi_hash": sha(root / protocol["canonical_abi"])
            == DUAL_PATH_PROGRESSIVE_ABI_SHA256,
            "target_parameters": target_parameters == 291_283_968,
    ...
    result = {
        "format": "layercake-postrelease-dual-path-progressive-construct-result/1",
        ...
        "claim_boundary": "Generic dual-path progressive host construct only; no external acquisition, quality, or performance claim.",
    }
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.command == "execute":
        if output.exists():
            raise RuntimeError("dual-path construct result immutable")
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    elif json.loads(output.read_text(encoding="utf-8")) != result:
        raise RuntimeError("stored dual-path construct differs")
