...
from .campaign_statistics import mean, median, p50, p95, p99
...
def validate_evidence_manifest(document: Mapping[str, Any], root: Path) -> dict[str, str]:
    ...
    raw_rows = sorted((path, digest) for path, digest in result.items() if "/raw_runs/" in path)
...
def derive_performance(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for key, group in sorted(groups.items()):
        ...
        throughputs = [row["output"]["target_bytes"] / row["timing"]["total_latency_seconds"] for row in group]
        ...
        summaries.append(
            {
                ...
                "trials": len(group),
                "latency_seconds": {"mean": mean(latencies), "median": median(latencies), "p50": p50(latencies), "p95": p95(latencies), "p99": p99(latencies)},
                "time_to_first_output_seconds": {"p50": p50(ttfo), "p95": p95(ttfo), "p99": p99(ttfo)},
                ...
                "tokens_per_second": {"mean": mean(token_rates), "p50": p50(token_rates)},
    ...
    return {"format": "layercake-phase1-derived-performance/1", "summaries": summaries}
...
def validate_phase1_bundle(
    ...
    discovered = {
        ...
        if path.is_file() and path.name not in lifecycle_names
    }
