...
class ModernCausalBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        return h + self.down(F.silu(self.gate(normalized)) * self.up(normalized))
...
class FusedModernCausalBlock(nn.Module):
    ...
    def decode_with_cache(
        ...
        attended = F.scaled_dot_product_attention(
            query, all_key, all_value, is_causal=False
        )
...
class CausalBytePatchLM(nn.Module):
    ...
    def __init__(
        ...
        patch_input_width = patch_size * d_byte + (
        ...
        if modern_blocks:
            ...
            self.core = nn.ModuleList(
                block_type(d_model, heads) for _ in range(layers)
            )
        else:
            block = nn.TransformerEncoderLayer(
                d_model, heads, d_model * 4, batch_first=True, norm_first=True
        ...
        if local_decoder == "gru":
            ...
            self.local_blocks = nn.ModuleList(
                CausalConvBlock(d_model, 2**index)
            ...
            if local_position_embeddings:
                self.local_pos = nn.Embedding(
                    max_patches * patch_size, d_model
                )
            if modern_blocks:
                block_type = (
                    FusedModernCausalBlock
        ...
        if tie_byte_embeddings:
            self.output_to_byte = nn.Linear(d_model, d_byte, bias=False)
        ...
        self.aux_heads = nn.ModuleList(
            nn.Linear(d_model, 256) for _ in range(mtp_depth)
        )
        if patch_prediction:
    ...
    def forward(
        ...
        local_in = torch.cat([byte_h, context], dim=-1)
    ...
    def cached_generation_step(
        ...
        first = (
            ...
            else first_logits.argmax(dim=-1)
        )
        ...
        second = (
            forced_patch[:, 1]
            if forced_patch is not None
            else second_logits.argmax(dim=-1)
        )
