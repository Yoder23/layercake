...
def _phase_dir(root: Path, phase: int) -> Path:
    if phase == 2:
        ...
        if campaign_path.is_file():
            ...
            record = campaign.get("phase_records", {}).get("phase2", {})
            if record.get("recertification_id") == PHASE2_RECERTIFICATION_ID:
                active = record.get("active_evidence_dir")
                if active != PHASE2_RECERTIFICATION_DIR.as_posix():
                    raise CampaignVerificationError(
                        "Phase 2 recertification evidence directory is not canonical"
                    )
                return _path(root, PHASE2_RECERTIFICATION_DIR)
...
def _phase_evidence_files(root: Path, phase: int) -> list[Path]:
    ...
    if phase == 6:
        try:
            from .evaluation.phase6_evidence import (
                Phase6EvidenceError,
                phase6_evidence_files,
            )

            return phase6_evidence_files(root)
    ...
    if phase == 8:
        try:
            from .evaluation.phase8_evidence import (
                Phase8EvidenceError,
                phase8_evidence_files,
            )

            return phase8_evidence_files(root)
        except Phase8EvidenceError as error:
            raise CampaignVerificationError(
                f"Phase 8 evidence manifest failed: {error}"
            ) from error
    excluded = {
...
def _verify_phase_evidence(root: Path, phase: int, contracts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    if phase == 6:
        try:
            ...
            summary = validate_phase6_bundle(
                root, _phase_dir(root, 6)
            )
            payload = read_document(
                _lifecycle_path(root, 6, "certificate_payload.json")
            )
            validate_required_gates(
                root, 6, payload, contracts["claim_contract.yaml"]
            )
            return summary
        except Phase6EvidenceError as error:
            raise CampaignVerificationError(
    ...
    if phase == 8:
        try:
            ...
            summary = validate_phase8_bundle(
                root, _phase_dir(root, 8)
            )
            payload = read_document(
                _lifecycle_path(root, 8, "certificate_payload.json")
            )
            validate_required_gates(
                root, 8, payload, contracts["claim_contract.yaml"]
            )
            return summary
        except Phase8EvidenceError as error:
            raise CampaignVerificationError(
                f"Phase 8 typed evidence failed: {error}"
            ) from error
    raise CampaignVerificationError(
...
def verify_sealed(root: Path, phase: int) -> dict[str, Any]:
    ...
    if phase == 6:
        try:
            ...
            validate_phase6_bundle(root, _phase_dir(root, 6))
        except Phase6EvidenceError as error:
            raise CampaignVerificationError(
    ...
    if phase == 8:
        try:
            ...
            validate_phase8_bundle(root, _phase_dir(root, 8))
        except Phase8EvidenceError as error:
            raise CampaignVerificationError(
                f"sealed Phase 8 typed evidence failed: {error}"
            ) from error
...
def _generation_trace_facts(trace_directory: Path) -> dict[str, int]:
    if not trace_directory.is_dir():
        raise CampaignVerificationError("promoted generation trace directory is missing")
    requests = 0
    generated_bytes = 0
    selected_from_logits = 0
    external_overrides = 0
    planner_calls = 0
    template_calls = 0
    retrieval_calls = 0
    stored_answer_calls = 0
    for path in sorted(trace_directory.glob("*.jsonl")):
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            try:
                ...
                raise CampaignVerificationError(
                    f"invalid generation trace {path.name}:{line_number}: {error}"
                ) from error
            if not isinstance(record, dict):
                raise CampaignVerificationError("generation trace request must be an object")
            try:
                prompt = bytes.fromhex(record["prompt_bytes_hex"])
                generated = bytes.fromhex(record["generated_bytes_hex"])
            except (KeyError, TypeError, ValueError) as error:
                raise CampaignVerificationError(
                    f"generation trace bytes are invalid in {path.name}:{line_number}"
                ) from error
            if hashlib.sha256(prompt).hexdigest() != record.get("prompt_sha256"):
                raise CampaignVerificationError("generation trace prompt hash mismatch")
            if hashlib.sha256(generated).hexdigest() != record.get("generated_sha256"):
                raise CampaignVerificationError("generation trace output hash mismatch")
            events = record.get("byte_events")
            if not isinstance(events, list) or len(events) != len(generated):
                raise CampaignVerificationError(
                    "generation trace must contain one event for every generated byte"
                )
            for index, (chosen, event) in enumerate(zip(generated, events)):
                if not isinstance(event, dict) or event.get("index") != index:
                    raise CampaignVerificationError("generation trace byte indexes are not canonical")
                if event.get("chosen_byte") != chosen:
                    raise CampaignVerificationError("generation trace chosen byte does not match output")
                summary = event.get("distribution_summary")
                if not isinstance(summary, dict) or not isinstance(summary.get("entropy_bits"), (int, float)):
                    raise CampaignVerificationError("generation trace lacks a decoding distribution summary")
                if event.get("source") == "model_logits":
                    selected_from_logits += 1
                if event.get("external_override") is True:
                    external_overrides += 1
            counters = record.get("external_path_counters")
            if not isinstance(counters, dict):
                raise CampaignVerificationError("generation trace lacks external-path counters")
            planner_calls += int(counters.get("planner_calls", -1))
            template_calls += int(counters.get("template_calls", -1))
            retrieval_calls += int(counters.get("retrieval_calls", -1))
            stored_answer_calls += int(counters.get("stored_answer_calls", -1))
            requests += 1
            generated_bytes += len(generated)
    if requests == 0 or generated_bytes == 0:
        raise CampaignVerificationError("promoted generation traces contain no generated bytes")
    return {
        "requests": requests,
        "total_generated_bytes": generated_bytes,
        "generated_bytes_selected_from_model_logits": selected_from_logits,
        "external_next_token_overrides": external_overrides,
        "planner_calls": planner_calls,
        "template_calls": template_calls,
        "retrieval_calls": retrieval_calls,
        "stored_answer_calls": stored_answer_calls,
    }


def evaluate_mission_guard(
    root: Path,
    *,
    certificate_path: Path,
    core_manifest_path: Path,
) -> dict[str, Any]:
    """Recompute permanent Phase 2 invariants from manifests and byte-level traces."""

    certificate = read_document(certificate_path)
    manifest = read_document(core_manifest_path)
    violations: list[str] = []
    architecture = " ".join(_walk_strings(manifest)).lower()
    prohibited_tokenizers = ("bpe", "sentencepiece", "wordpiece", "unigram tokenizer")
    if any(marker in architecture for marker in prohibited_tokenizers):
        violations.append("INVALID_EVIDENCE_NOT_TOKENIZER_FREE")
    input_contract = manifest.get("input_contract")
    permitted_inputs = {"raw_utf8_bytes", "deterministic_byte_patches"}
    if (
        not isinstance(input_contract, dict)
        or input_contract.get("kind") not in permitted_inputs
        or input_contract.get("learned_tokenizer") is not False
    ):
        violations.append("TOKENIZER_FREE_CONTRACT_NOT_PROVEN")
    prohibited_generation = (
        "planner", "template", "forced token", "forced_token", "retrieval",
        "stored answer", "stored_answer", "predetermined", "teacher-forced",
    )
    if any(marker in architecture for marker in prohibited_generation):
        violations.append("INVALID_EVIDENCE_DETERMINISTIC_PLANNER")
    generation = manifest.get("generation_contract")
    if (
        not isinstance(generation, dict)
        or generation.get("continuation_source") != "model_logits"
        or generation.get("external_next_byte_allowed") is not False
    ):
        violations.append("FREE_NEURAL_GENERATION_CONTRACT_NOT_PROVEN")
    extension = manifest.get("extension_interface")
    if (
        not isinstance(extension, dict)
        or not isinstance(extension.get("version"), str)
        or not extension.get("version")
    ):
        violations.append("VERSIONED_EXTENSION_INTERFACE_MISSING")

    facts: dict[str, int] = {}
    trace_relative = manifest.get("generation_trace_directory")
    if not isinstance(trace_relative, str):
        violations.append("GENERATION_TRACES_MISSING")
    else:
        try:
            facts = _generation_trace_facts(_path(root, trace_relative))
        except CampaignVerificationError as error:
            violations.append(f"GENERATION_TRACE_INVALID:{error}")
    if facts:
        if facts["generated_bytes_selected_from_model_logits"] != facts["total_generated_bytes"]:
            violations.append("GENERATED_BYTES_NOT_SELECTED_FROM_MODEL_LOGITS")
        for field in (
            "external_next_token_overrides", "planner_calls", "template_calls",
            "retrieval_calls", "stored_answer_calls",
        ):
            if facts[field] != 0:
                violations.append(f"{field.upper()}_NONZERO")

    lineage = certificate.get("lineage")
    if not isinstance(lineage, dict):
        lineage = {}
    primary = lineage.get("primary_checkpoint_sha256") or certificate.get(
        "primary_checkpoint_sha256"
    )
    quality_checkpoint = certificate.get("quality_checkpoint_sha256")
    performance_checkpoint = certificate.get("performance_checkpoint_sha256")
    if not (
        isinstance(primary, str)
        and len(primary) == 64
        and primary == quality_checkpoint == performance_checkpoint
    ):
        violations.append("QUALITY_SPEED_CHECKPOINT_MISMATCH")
    comparator_quality = certificate.get("comparator_quality_checkpoint_sha256")
    comparator_speed = certificate.get("comparator_performance_checkpoint_sha256")
    if not (
        isinstance(comparator_quality, str)
        and len(comparator_quality) == 64
        and comparator_quality == comparator_speed
    ):
        violations.append("COMPARATOR_CHECKPOINT_MISMATCH")
    comparator_manifest_relative = lineage.get("transformer_comparator_manifest")
    if not isinstance(comparator_manifest_relative, str):
        violations.append("OPTIMIZED_TRANSFORMER_COMPARATOR_NOT_PROVEN")
    else:
        try:
            comparator = read_document(_path(root, comparator_manifest_relative))
        except CampaignVerificationError:
            violations.append("OPTIMIZED_TRANSFORMER_COMPARATOR_NOT_PROVEN")
        else:
            runtime = comparator.get("runtime")
            if (
                not isinstance(runtime, dict)
                or runtime.get("optimized_cpu") is not True
                or runtime.get("execution") == "eager_python"
            ):
                violations.append("OPTIMIZED_TRANSFORMER_COMPARATOR_NOT_PROVEN")
            provenance = comparator.get("conversion_provenance")
            if (
                not isinstance(provenance, dict)
                or not isinstance(provenance.get("source_checkpoint_sha256"), str)
                or not isinstance(provenance.get("converted_weight_sha256"), str)
                or provenance.get("numerical_verification_passed") is not True
            ):
                violations.append("TRANSFORMER_CONVERSION_PROVENANCE_NOT_PROVEN")
    return {
        "format": "layercake-mission-guard/1",
        "status": "PASS" if not violations else "FAIL",
        "passed": not violations,
        "certificate": certificate_path.relative_to(root).as_posix(),
        "core_manifest": core_manifest_path.relative_to(root).as_posix(),
        "trace_facts": facts,
        "violations": sorted(set(violations)),
    }


def mission_guard(root: Path) -> dict[str, Any]:
    campaign = load_contracts(root)["campaign.yaml"]
    record = campaign.get("phase_records", {}).get("phase2", {})
    if record.get("recertification_id") == PHASE2_RECERTIFICATION_ID:
        certificate_path = _path(
            ...
            record.get(
                "certificate",
                PHASE2_RECERTIFICATION_DIR / "release_certificate.json",
            ),
        )
        manifest_path = _path(
            root,
            record.get(
                "core_manifest",
                PHASE2_RECERTIFICATION_ARTIFACT_DIR / "final_tokenizer_free_core/manifest.json",
            ),
        )
    else:
        certificate_path = _path(root, "results/moonshot/phase2/release_certificate.json")
        manifest_path = _path(root, "artifacts/moonshot/phase2/final_core/manifest.json")
    return evaluate_mission_guard(
        root, certificate_path=certificate_path, core_manifest_path=manifest_path
    )


def _source_evidence(root: Path, relative: str, patterns: Sequence[str]) -> dict[str, Any]:
    path = _path(root, relative)
...
def _validate_phase2_recertification_reopen_state(campaign: Mapping[str, Any]) -> None:
    if campaign.get("current_phase") != 3:
        raise CampaignVerificationError("Phase 2 recertification requires current_phase 3")
    phases = campaign.get("phases", {})
    expected = {
        PHASE_KEYS[0]: "SEALED",
        PHASE_KEYS[1]: "SEALED",
    ...
    for key, value in expected.items():
        if phases.get(key) != value:
            raise CampaignVerificationError(
                f"Phase 2 recertification requires {key}={value}, got {phases.get(key)}"
            )
    for key in PHASE_KEYS[4:]:
        if phases.get(key) != "LOCKED":
            raise CampaignVerificationError(
                f"Phase 2 recertification refuses non-LOCKED future phase {key}"
            )


def reopen_phase2_recertification(root: Path) -> dict[str, Any]:
    """Invalidate the non-tokenizer-free planner proof and reopen Phase 2 only."""

    contracts = load_contracts(root)
    campaign = contracts["campaign.yaml"]
    _validate_phase2_recertification_reopen_state(campaign)
    if _git(root, "status", "--porcelain=v1"):
        raise CampaignVerificationError("Phase 2 recertification reopen requires a clean worktree")
    verified = verify_sealed(root, 2)
    guard = mission_guard(root)
    required = {
        "INVALID_EVIDENCE_DETERMINISTIC_PLANNER",
        "INVALID_EVIDENCE_NOT_TOKENIZER_FREE",
    }
    if not required.issubset(set(guard["violations"])):
        raise CampaignVerificationError(
            "existing Phase 2 evidence did not reproduce both mandatory invalidation reasons"
        )
    old_record = json.loads(json.dumps(campaign["phase_records"]["phase2"]))
    tag = str(verified["completion_tag"])
    tagged_certificate = _git_blob(
        root, tag, "results/moonshot/phase2/release_certificate.json"
    )
    invalidation = {
        "format": "layercake-phase2-invalidation/1",
        "recertification_id": PHASE2_RECERTIFICATION_ID,
        "status": "INVALIDATED_SUPERSEDED_EVIDENCE_PRESERVED",
        "invalidated_tag": tag,
        "invalidated_tag_commit": verified["tag_commit"],
        "invalidated_release_commit": verified["release_commit"],
        "invalidated_certificate_path": "results/moonshot/phase2/release_certificate.json",
        "invalidated_certificate_sha256_tagged_blob": hashlib.sha256(
            tagged_certificate
        ).hexdigest(),
        "invalidated_certificate_sha256_recorded": old_record["certificate_sha256"],
        "invalidation_reasons": sorted(required),
        "mission_guard": guard,
        "source_evidence": [
            _source_evidence(
                root,
                "layercake/phase2_campaign.py",
                ("planned_ids", "planned_text", "next_token=selected", "forced_plan_tokens"),
            ),
            _source_evidence(
                root,
                "layercake/models/sparse_bpe_layercake.py",
                ("phase2_english_planner", "constrained_english_planner", "next_token"),
            ),
            _source_evidence(
                root,
                "layercake/models/phase2_english_planner.py",
                ("planner", "realize_english"),
            ),
        ],
        "potentially_valid_non_headline_measurements": [
            "checkpoint held-out loss under the learned BPE contract",
            "physical top-1 expert dispatch",
            "persistent per-layer KV state",
            "model parameter and checkpoint byte counts",
        ],
        "prohibited_future_headline_claims": [
            "tokenizer-free quality",
            "autonomous free-generation quality",
            "generation throughput",
            "generation latency",
            "time to first useful output",
            "matched-quality CPU advantage",
            "functional quality from planner-generated responses",
        ],
        "historical_evidence_preserved_at": "results/moonshot/phase2",
        "superseding_tag": "layercake-moonshot-phase2-r2",
    }
    invalidation_path = _path(root, PHASE2_RECERTIFICATION_DIR / "invalidation.json")
    _atomic_write(invalidation_path, invalidation)

    task_state_path = _path(root, PHASE2_RECERTIFICATION_DIR / "task_state.json")
    task_state = read_document(task_state_path)
    task_state.update({
        "updated_at_utc": "2026-07-23T00:00:00Z",
        "mission_status": "ACTIVE",
        "current_stage": "PHASE2_BASELINE_AUDIT",
        "phase2_status": "OPEN_RECERTIFICATION",
        "phase3_status": "LOCKED",
        "invalidation_path": invalidation_path.relative_to(root).as_posix(),
        "invalidation_sha256": sha256_file(invalidation_path),
    })
    _atomic_write(task_state_path, task_state)

    campaign.setdefault("invalidation_history", []).append({
        "phase": 2,
        "recertification_id": PHASE2_RECERTIFICATION_ID,
        "invalidated_tag": tag,
        "invalidated_tag_commit": verified["tag_commit"],
        "invalidated_certificate_sha256": hashlib.sha256(tagged_certificate).hexdigest(),
        "reasons": sorted(required),
        "record": invalidation_path.relative_to(root).as_posix(),
        "record_sha256": sha256_file(invalidation_path),
    })
    campaign["current_phase"] = 2
    campaign["phases"][PHASE_KEYS[2]] = "OPEN"
    campaign["phases"][PHASE_KEYS[3]] = "LOCKED"
    for key in PHASE_KEYS[4:]:
        campaign["phases"][key] = "LOCKED"
    campaign["lineage"].update({
    ...
    campaign["phase_records"]["phase2"] = {
        "recertification_id": PHASE2_RECERTIFICATION_ID,
        "active_evidence_dir": PHASE2_RECERTIFICATION_DIR.as_posix(),
        "active_artifact_dir": PHASE2_RECERTIFICATION_ARTIFACT_DIR.as_posix(),
        "completion_tag": "layercake-moonshot-phase2-r2",
        "historical_record": old_record,
        "invalidation": invalidation_path.relative_to(root).as_posix(),
        "invalidation_sha256": sha256_file(invalidation_path),
        "superseded_tag": tag,
        "superseded_tag_commit": verified["tag_commit"],
        "superseded_release_commit": verified["release_commit"],
        "inherited_model_source_commit": campaign["lineage"]["model_source_commit"],
        "governance_commit": campaign["lineage"]["governance_commit"],
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subcommands.add_parser(
        "reopen-phase2-recertification",
        help="invalidate the BPE/planner proof and reopen tokenizer-free Phase 2 recertification",
    )
    subcommands.add_parser(
        "mission-guard",
        help="verify tokenizer-free autonomous generation and matched-checkpoint invariants",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "reopen-phase2-recertification":
            result = reopen_phase2_recertification(root)
        elif args.command == "mission-guard":
            result = mission_guard(root)
    ...
    if args.command == "verify-all" and result.get("completed_phases_valid") is not True:
        return 1
    if args.command == "mission-guard" and result.get("passed") is not True:
        return 1
    return 0
