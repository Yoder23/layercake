...
from layercake_extensions.progressive_replacement_core import (
    PROGRESSIVE_REPLACEMENT_CAPABILITIES,
    PROGRESSIVE_REPLACEMENT_CORE_ABI_SHA256,
    PROGRESSIVE_REPLACEMENT_CORE_ABI_VERSION,
    ProgressiveReplacementCoreHost,
    progressive_replacement_manifest_architecture,
)
from tests.models.test_decoder_direct_neural_core import _doc


def _model(tokenizer: DecoderAwareExternalTokenizer) -> ProgressiveReplacementCore:
    return ProgressiveReplacementCore(
...
def _package(tmp_path: Path):
    ...
    manifest = CakeManifest(
        schema_version="1",
        cake_id="progressive-replacement-core-construct",
        name="Progressive Replacement Core",
        ...
        architecture=progressive_replacement_manifest_architecture(model, tokenizer),
        supported_precisions=("fp32",),
        supported_backends=("pytorch", "cuda"),
        minimum_host_capabilities={"features": sorted(PROGRESSIVE_REPLACEMENT_CAPABILITIES)},
        tensor_payload_hash="",
    ...
    path = build_package(tmp_path / "progressive-replacement-core.cake", manifest, model.state_dict(), private_key=private)
    return path, public, key_id, state_dict_hash(model.state_dict()), tokenizer


def test_progressive_replacement_parameter_accounting_matches_target() -> None:
    expected = ProgressiveReplacementCore.parameter_count_for_config(
    ...
    assert expected == 253_836_288
    tokenizer = DecoderAwareExternalTokenizer(_doc())
    fixture = _model(tokenizer)
    assert fixture.parameter_count() == ProgressiveReplacementCore.parameter_count_for_config(
        fixed_vocab_size=tokenizer.vocab_size,
        full_width=24,
        bottleneck_width=8,
        replacement_layers=2,
...
def test_progressive_replacement_incremental_logits_match_full_forward() -> None:
...
def test_v8_signed_lifecycle_persistent_state_and_zero_learning(tmp_path: Path) -> None:
    package, public, key_id, state_hash, _ = _package(tmp_path)
    archive = package.read_bytes()
    host = ProgressiveReplacementCoreHost(tmp_path / "registry", trust_store={key_id: public})
...
def test_same_v8_package_executes_cuda(tmp_path: Path) -> None:
    package, public, key_id, state_hash, _ = _package(tmp_path)
    host = ProgressiveReplacementCoreHost(tmp_path / "registry", trust_store={key_id: public}, device="cuda")
