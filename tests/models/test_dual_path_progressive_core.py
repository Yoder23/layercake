...
from layercake_extensions.dual_path_progressive_core import (
    ...
    DUAL_PATH_PROGRESSIVE_CAPABILITIES,
    DualPathProgressiveCoreHost,
    dual_path_progressive_manifest_architecture,
)
from tests.models.test_decoder_direct_neural_core import _doc


def _model(tokenizer: DecoderAwareExternalTokenizer) -> DualPathProgressiveCore:
    return DualPathProgressiveCore(
...
def _package(tmp_path: Path):
    ...
    manifest = CakeManifest(
        schema_version="1",
        cake_id="dual-path-progressive-construct",
        name="Dual Path Progressive Core",
        description="construct",
        version="10.0.0",
        publisher={"id": "construct", "name": "Construct", "key_id": key_id},
        abi_version=DUAL_PATH_PROGRESSIVE_ABI_VERSION,
        abi_hash=DUAL_PATH_PROGRESSIVE_ABI_SHA256,
        ...
        architecture=dual_path_progressive_manifest_architecture(model, tokenizer),
        supported_precisions=("fp32",),
        supported_backends=("pytorch", "cuda"),
        minimum_host_capabilities={"features": sorted(DUAL_PATH_PROGRESSIVE_CAPABILITIES)},
        tensor_payload_hash="",
    ...
    path = build_package(
        tmp_path / "dual-path-progressive.cake",
        manifest,
        model.state_dict(),
        private_key=private,
    )
...
def test_dual_path_parameter_accounting_matches_materialized_fixture() -> None:
    tokenizer = DecoderAwareExternalTokenizer(_doc())
    model = _model(tokenizer)
    expected = DualPathProgressiveCore.parameter_count_for_config(
        fixed_vocab_size=tokenizer.vocab_size,
        full_width=24,
        bottleneck_width=8,
        replacement_layers=2,
    ...
    assert DualPathProgressiveCore.parameter_count_for_config(
...
def test_dual_path_prefill_and_incremental_match_full_forward() -> None:
...
def test_v10_signed_lifecycle_and_zero_learning(tmp_path: Path) -> None:
    package, public, key_id, state_hash, _ = _package(tmp_path)
    host = DualPathProgressiveCoreHost(tmp_path / "registry", trust_store={key_id: public})
...
def test_same_v10_package_executes_cuda(tmp_path: Path) -> None:
    package, public, key_id, state_hash, _ = _package(tmp_path)
    host = DualPathProgressiveCoreHost(
