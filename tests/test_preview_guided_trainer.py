...
def test_preview_guided_trainer_commit_and_rollback(tmp_path):
    ...
    commit, cert, rollback, preview, syllabus = trainer.run_preview_guided(rubric, data, model=None, parent_commit=parent, train_step=lambda: 0.9, eval_fn=lambda: {"score": 1.0}, certificate_path=tmp_path / "cert.json")
    assert cert.passed and commit.commit_id and preview.preview_id and syllabus.syllabus_id
    values = iter([1.0, 9.0])
    failed, failed_cert, rollback, *_ = trainer.run_preview_guided(rubric, data, model=None, parent_commit=commit, train_step=lambda: next(values), eval_fn=lambda: {"score": 1.0}, certificate_path=tmp_path / "bad.json")
