...
PREREGISTRATION = (
    ROOT
    / "moonshot"
    / "phase4_semantic_transition_identity_codec_preregistration.json"
)
PARENT = (
    ROOT
    / "artifacts"
...
def _new_model_from_parent() -> tuple[
    SemanticActionPlanResidual, dict[str, Any]
]:
    ...
    if unexpected or set(missing) != TRAINABLE_TENSORS:
        raise RuntimeError(
            "transition codec initialization boundary mismatch: "
            f"missing={missing} unexpected={unexpected}"
        )
    return model, parent_artifact
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if _sha256(CACHE) != CACHE_SHA256:
        raise RuntimeError("lexical cache identity changed")
    if _sha256(LEXICAL_DATASET) != LEXICAL_DATASET_SHA256:
    ...
    model, parent_artifact = _new_model_from_parent()
    ...
    if trainable_count != 788480:
        raise RuntimeError(
            f"transition codec parameter count changed: {trainable_count}"
        )
    ...
    for step in range(1, args.steps + 1):
        ...
        logits = F.linear(result["adapted"][pointer_valid], embedding)
        ...
        if step == 1 or step % 50 == 0:
            record = {
                "step": step,
                "pointer_realization_cross_entropy": loss_value,
                "wall_seconds": time.perf_counter() - started,
            }
            curves.append(record)
            print(json.dumps(record), flush=True)
    if best_state is None:
        raise RuntimeError("training produced no transition codec")
    ...
    training = {
        ...
        "best_pointer_realization_cross_entropy": best_loss,
    ...
    for index, row in enumerate(rows):
        ...
        if source:
            try:
                ...
                exec(compile(source, "<transition-codec>", "exec"), namespace)
                function = namespace[row["function_name"]]
                functional = all(
                    function(value) == value
                    for value in (0, "identity", [1, 2, 3])
                )
            except Exception:
                functional = False
        records.append(
...
def evaluate_python(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output.exists():
        raise RuntimeError(f"immutable output already exists: {output}")
    lexical_path = _rooted(args.lexical_evidence)
    lexical = json.loads(lexical_path.read_text(encoding="utf-8"))
    if lexical.get("status") != "PASS":
        raise RuntimeError("Python screen is locked until lexical screen passes")
    if lexical.get("artifact_file_sha256") != _sha256(artifact_path):
        raise RuntimeError("lexical evidence belongs to another artifact")
    device = _device(args.device)
    core, tokenizer, cake, artifact, core_metadata = _load_for_evaluation(
        artifact_path, device
    )
    ...
    evidence = {
        ...
        "protocol": PREREGISTRATION.relative_to(ROOT).as_posix(),
        "protocol_sha256": _sha256(PREREGISTRATION),
        "lexical_evidence": lexical_path.relative_to(ROOT).as_posix(),
        "lexical_evidence_sha256": _sha256(lexical_path),
        "artifact": artifact_path.relative_to(ROOT).as_posix(),
        "artifact_file_sha256": _sha256(artifact_path),
        "payload_hash": artifact["payload_hash"],
        "abi_version": artifact["abi_version"],
...
def parse_args() -> argparse.Namespace:
    ...
    train_parser.add_argument("--seed", type=int, default=10540)
